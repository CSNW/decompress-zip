...
DecompressZip.prototype.extractFiles = function (files, options, results) {
    ...
    var startFiles = files.slice(0, concurrencyLimit);
    var fileIndex = startFiles.length - 1;
    startFiles.forEach(function (file) {
        promises.push(addPromise(file));
    });

    function addPromise(file) {
        var promise = self.extractFile(file, options)
        .then(function (result) {
            self.emit('progress', completedCount++, files.length);
            results.push(result);

            // chain on new promises while there are files left
            fileIndex++;
            if (fileIndex < files.length)
                return addPromise(files[fileIndex]);
        });
        return promise;
    }
